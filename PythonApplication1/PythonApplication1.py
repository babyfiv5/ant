...
class ant_colony:
    class ant(Thread):
        def __init__(self, init_location, possible_locations, pheromone_map, distance_callback, alpha, beta,
            ...
            初始化一个蚂蚁，遍历地图
            init_location - >标记蚂蚁在地图上的起始位置
            possible_locations - >蚂蚁可以到达的可能节点列表 当在内部使用时，给出蚂蚁可以遍历_minus已经访问的节点的可能位置的列表_
            pheromone_map - >映射每个节点之间每次遍历的信息素值
            distance_callback - >是计算两个节点之间距离的函数
            alpha - > ACO算法中的一个参数来控制在_pick_path（）中做出选择时的信息素量的影响。
            beta - >一个来自ACO的参数，控制距离对_pick_path（）中下一个节点的影响
            first_pass - >如果这是地图上的第一遍，那么采取一些不同的步骤，如下所述
            route - >用蚂蚁遍历的节点的标签更新的列表
            pheromone_trail - >沿着蚂蚁路径存放的信息素量的列表，映射到路径中的每个遍历
            distance_traveled - >沿着路线上的步骤转过的总距离 位置 - >标记蚂蚁现在在哪里
            tour_complete - >标志来表明蚂蚁已经完成了它的遍历 由get_route（）和get_distance_traveled（）使用 “””
            """
        ...
        def run(self):
            """
            until self.possible_locations is empty (the ant has visited all nodes运行直到蚂蚁已经访问了所有的节点)
                _pick_path() to find a next node to traverse to 找到要遍历的下一个节点
                _traverse() to:
                    _update_route() (to show latest traversal显示最新的遍历)
                    _update_distance_traveled() (after traversal遍历之后更新距离)
            return the ants route and its distance, for use in ant_colony:
                do pheromone updates 返回蚂蚁的路线和距离，用于ant_colony：做信息素更新
                check for new possible optimal solution with this ants latest tour检查这个蚂蚁最新的旅游新的可能的最佳解决方案
            """
        ...
        def _pick_path(self):
            ...
            实现ACO的路径选择算法，计算从当前位置的每个可能转换的吸引力，然后根据其吸引力随机选择下一个路径
            """
        ...
        def _traverse(self, start, end):
            ...
            _update_distance_traveled() to record new distance traveled 记录新的行驶距离
    ...
    def __init__(self, nodes, distance_callback, start=None, ant_count=50, alpha=.5, beta=1.2,
        ...
        初始化一个蚁群（安置一些工蚁，它将遍历一张地图，按照ACO [Ant Colony Optimization]找到最优路径）
        ...
        nodes -> is assumed to be a dict() mapping node ids to values
            that are understandable by distance_callback 被假定为dict（）映射节点id到distance_callback可理解的值
